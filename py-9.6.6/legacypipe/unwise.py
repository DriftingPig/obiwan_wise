...
'''
#obiwan, adding simcat, targetwcs
def unwise_forcedphot(cat, tiles, band=1, roiradecbox=None,
                      ...
                      get_masks=None,
                      move_crpix=False,
                      modelsky_dir=None,simcat=None,targetwcs=None):
    '''
    ...
    *get_masks*: the WCS to resample mask bits into.
    ...
    for tile in tiles:
        ...
        tim.tile = tile
        #obiwan, add_source_to_tractor_tim
        from legacypipe.unwise_sim import add_source_to_tractor_tim
        tim = add_source_to_tractor_tim(tim,band,simcat,targetwcs)
        tims.append(tim)
    ...
    #obiwan, get_sim_models
    if simcat is None:
        get_sim_models=False
    else:
        get_sim_models=True
    if get_sim_models:
        sim_models = []
        for i,tim in enumerate(tims):
            if band==1 or band==2:
                tile = tim.tile
                sim_models.append((tile.coadd_id, band, tim.wcs.wcs, tim.sim_image, tim.sim_image,
                           tim.sim_image_ivar))
